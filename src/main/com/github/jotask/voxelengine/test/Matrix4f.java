...
public class Matrix4f {
    ...
    public Matrix4f set (Vector3 xAxis, Vector3 yAxis, Vector3 zAxis, Vector3 pos) {
        val[M00] = xAxis.x;
        val[M01] = xAxis.y;
        val[M02] = xAxis.z;
        val[M10] = yAxis.x;
     ...
     * @return This matrix for the purpose of chaining methods together. */
    public Matrix4f trn (Vector3 vector) {
        val[M03] += vector.x;
        val[M13] += vector.y;
        val[M23] += vector.z;
        return this;
     ...
     * @return This matrix for the purpose of chaining methods together. */
    public Matrix4f setTranslation (Vector3 vector) {
        val[M03] = vector.x;
        val[M13] = vector.y;
        val[M23] = vector.z;
        return this;
     ...
     * @param vector The translation vector
     * @return This matrix for the purpose of chaining methods together. */
    public Matrix4f setToTranslation (Vector3 vector) {
        idt();
        val[M03] = vector.x;
        val[M13] = vector.y;
        val[M23] = vector.z;
     ...
     * @return This matrix for the purpose of chaining methods together. */
    public Matrix4f setToTranslationAndScaling (Vector3 translation, Vector3 scaling) {
        idt();
    ...
    public Matrix4f setToScaling (Vector3 vector) {
        idt();
        val[M00] = vector.x;
        val[M11] = vector.y;
        val[M22] = vector.z;
     ...
     * @return This matrix for the purpose of chaining methods together. */
    public Matrix4f setToLookAt (Vector3 direction, Vector3 up) {
    ...
    static final Matrix4f tmpMat = new Matrix4f();
     ...
     * @param up the up vector
     * @return This matrix */
    public Matrix4f setToLookAt (Vector3 position, Vector3 target, Vector3 up) {
    ...
    public Matrix4f setToWorld (Vector3 position, Vector3 forward, Vector3 up) {
     ...
     * @return This matrix for chaining */
    public Matrix4f setAsAffine (Matrix4f mat) {
        val[M00] = mat.val[M00];
        val[M10] = mat.val[M10];
        val[M01] = mat.val[M01];
        val[M11] = mat.val[M11];
    ...
    public Matrix4f scl (Vector3 scale) {
        val[M00] *= scale.x;
        val[M11] *= scale.y;
        val[M22] *= scale.z;
        return this;
    ...
    public float[] mul (float[] mata, float[] matb) {
...
//        System.out.println(new Matrix4f(tmp));
	...
	*/
     ...
     * same as {@link Vector3#prj(Matrix4f)}.
     * @param mat the matrix
     * @param vec the vector. */
	...
	*/
     ...
     * major matrix as you can get from {@link Matrix4f#val}. The vector array is assumed to hold a 3-component vector, with x being
     * the first element, y being the second and z being the last component. The result is stored in the vector array. This is the
     * same as {@link Vector3#rot(Matrix4f)}.
     * @param mat the matrix
     * @param vec the vector. */
	...
	*/
     ...
     * @param translation
     * @return This matrix for the purpose of chaining methods together. */
    public Matrix4f translate (Vector3 translation) {
